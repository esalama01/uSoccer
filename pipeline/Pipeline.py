...
class MatchScraper: # A specific crawler that is designed to be compatible with whoscored's html.
    ...
    def crawl(self): #This returns the match event data in json format
        ...
        all_match_data = []
        for base_url in tqdm(self.base_urls, desc="Downloading Matches"):
            try:
                ...
                all_match_data.append({'data': data, 'url': base_url})

            except Exception as e:
                print(f"\n(!) Connection/Parsing error on {base_url}: {e}")
                continue
                ...
                if element['name'].lower() == self.country.lower():
                    for tournament in element['tournaments']:
                        if tournament['name'].lower() == self.name.lower():
                            return tournament['url']  # Return directly and exit
        else:
            for element in leagues:
                for tournament in element['tournaments']:
                    if tournament['name'].lower() == self.name.lower():
...
def get_infos(url):
    # Try the long URL format first (case-insensitive for 'matches/')
    long_pattern = re.compile(r'(?i)matches/(\d+)/.*?/[^-]+-(.+?)-(\d{4}-\d{4})')
    match_long = long_pattern.search(url)

    if match_long:
        return match_long.group(1), match_long.group(2), match_long.group(3)

    # Fallback for the short URL format generated by LeagueScraper
    short_pattern = re.compile(r'(?i)matches/(\d+)')
    match_short = short_pattern.search(url)

    if match_short:
        return match_short.group(1), "Unknown", "Unknown"
